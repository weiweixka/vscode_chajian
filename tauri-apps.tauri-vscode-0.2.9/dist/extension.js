...
// node_modules/.pnpm/run-in-terminal@0.0.3/node_modules/run-in-terminal/lib/index.js
var require_lib = __commonJS({
  "node_modules/.pnpm/run-in-terminal@0.0.3/node_modules/run-in-terminal/lib/index.js"(exports2) {
    "use strict";
    var child_process_1 = require("child_process");
    var path_1 = require("path");
    var util_1 = require("util");
    function runInTerminal2(command, args, options) {
      return _terminalSpawn(command, args || [], options).then(function(value) {
        var result = value;
        result.kill2 = function() {
          return _terminate(value);
        };
        return result;
      });
    }
    exports2.runInTerminal = runInTerminal2;
    var _terminalSpawn = /^win/.test(process.platform) ? runInTerminalWin : process.platform === "darwin" ? runInTerminalMac : runInTerminalLinux;
    var _terminate = /^win/.test(process.platform) ? terminateWin : terminateMacLinux;
    ...
    function terminateWin(process2) {
      if (process2) {
        return new Promise(function(resolve, reject) {
          process2.once("exit", function(code, signal) {
            resolve(code);
          });
          child_process_1.exec("taskkill /F /T /PID " + process2.pid, function(err, stdout, stderr) {
            if (err) {
              reject(err);
            }
          });
    ...
    function terminateMacLinux(process2) {
      if (process2) {
        return new Promise(function(resolve, reject) {
          process2.once("exit", function(code, signal) {
            resolve(code);
...
var import_run_in_terminal = __toESM(require_lib());
...
ignored_fn = function(path3) {
  return this.seen.has(path3) || !!__privateGet(this, _ignore)?.ignored?.(path3);
};
childrenIgnored_fn = function(path3) {
  return !!__privateGet(this, _ignore)?.childrenIgnored?.(path3);
};
...
function registerSchemasHandler(context) {
    ...
    new class {
      ...
      async provideTextDocumentContent(uri) {
        if (uri.authority === "schemas" && uri.path === "/config.json") {
          const schemaFile = (await vscode.workspace.findFiles(
            "**/node_modules/@tauri-apps/cli/{schema.json,config.schema.json}"
          ))[0];
          if (schemaFile) return fs.readFileSync(schemaFile.fsPath, "utf-8");
          async function getSchemaFromRelease(version) {
            const filename = version.startsWith("1") ? "schema.json" : "config.schema.json";
            const res2 = await axios_default.get(
              `https://github.com/tauri-apps/tauri/releases/download/tauri-build-v${version}/${filename}`
            );
            return res2.status == 200 ? JSON.stringify(res2.data) : null;
          }
          const cargoLockPath = (await vscode.workspace.findFiles("**/Cargo.lock"))[0];
          if (cargoLockPath) {
          ...
          const cargoTomlPath = (await vscode.workspace.findFiles("**/Cargo.toml"))[0];
          if (cargoTomlPath) {
            ...
            for (const regex2 of [
              ...
              const matches = regex2.exec(cargoToml);
              if (matches && matches[1]) {
                const schema = await getSchemaFromRelease(matches[1]);
                if (schema) return schema;
              }
          ...
          let tag_name = res.data.find(
            (e) => e.tag_name.startsWith("tauri-build-v")
          )?.tag_name;
          if (tag_name) {
            const matches = /((\d|x|\*)+\.(\d|x|\*)+\.(\d|x|\*)+(-[a-zA-Z-0-9]*(.[0-9]+))*)/g.exec(
              tag_name
            );
            if (matches && matches[1]) {
              const schema = await getSchemaFromRelease(matches[1]);
  ...
  __pickProjectAndRunTauriScript(
    (projectPath) => {
      ...
      if (__isVueCliApp(projectPath)) {
        ...
        installCommand = __usePnpm(projectPath) ? "pnpm add -D @tauri-apps/cli" : __useYarn(projectPath) ? "yarn add @tauri-apps/cli --dev" : `${__getNpmBin()} install @tauri-apps/cli --save-dev`;
        onInstall = () => {
    ...
    () => {
      ...
      return paths.filter((p) => {
        return fs.existsSync(path2.join(p, "src-tauri"));
      });
    }
  );
}
function runTauriDev() {
  __pickProjectAndRunTauriScript(
    (projectPath) => __runTauriScript(["dev"], { cwd: projectPath })
  );
}
function runTauriBuild() {
  __pickProjectAndRunTauriScript(
    (projectPath) => __runTauriScript(["build"], { cwd: projectPath })
  );
}
function runTauriBuildDebug() {
  __pickProjectAndRunTauriScript(
    (projectPath) => __runTauriScript(["build", "--debug"], { cwd: projectPath })
  );
}
...
function __isNodeProject(cwd) {
  return fs.existsSync(path2.join(cwd, "package.json"));
}
function __getPackageJson(appDir) {
  const packageJsonPath = path2.join(appDir, "package.json");
...
function __getNpmProjectsPaths() {
  ...
  for (const folder of folders) {
    const npmProjectRoots = glob.sync(folder.uri.fsPath.split("\\").join("/") + "/**/package.json").map((p) => path2.dirname(p));
...
function __getTauriProjectsPaths() {
  ...
  const paths = [];
  for (const folder of folders) {
    const tauriProjectRoots = glob.sync(folder.uri.fsPath.split("\\").join("/") + "/**/src-tauri").map((p) => path2.dirname(p));
...
function __runCommandInTerminal(command, args, cwd) {
  return (0, import_run_in_terminal.runInTerminal)(command, args, { cwd, env: process.env }).then(
    (process2) => {
      return new Promise((resolve, reject) => {
...
function __runCommandInOutputWindow(command, args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    const p = (0, import_child_process.exec)(cmd, { cwd, env: process.env });
    if (p.pid === void 0) {
      return reject();
    }
    runningProcesses.set(p.pid, { process: p, cmd });
...
function __usePnpm(projectPath) {
  return fs.existsSync(path2.join(projectPath, "pnpm-lock.yaml"));
}
function __useYarn(projectPath) {
  return fs.existsSync(path2.join(projectPath, "yarn.lock"));
}
function __useNpm(projectPath) {
  return fs.existsSync(path2.join(projectPath, "package-lock.json"));
}
function __useCargo() {
  try {
    (0, import_child_process.execSync)("cargo tauri --version", { windowsHide: true });
    return true;
  } catch {
    return false;
  }
...
function __getPackageManagerCommand(projectPath) {
  const m = __usePnpm(projectPath) ? "pnpm" : __useYarn(projectPath) ? "yarn" : __useNpm(projectPath) ? __getNpmCommand() : __useCargo() ? "cargo" : null;
  if (!m) {
